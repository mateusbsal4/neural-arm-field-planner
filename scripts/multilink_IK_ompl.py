...
class IK_Controller:
    ...
    def plan_path(self):
        ...
        target_pos_left = self.goal_pos - np.array([0.0, 0.015, 0])
        target_pos_right = self.goal_pos + np.array([0.0, 0.015, 0])
        ...
        qpos = self.franka.inverse_kinematics_multilink(
            links    = [self.left_finger, self.right_finger],
        ...
        pos_left = self.left_finger.get_pos()
        pos_right = self.right_finger.get_pos()
