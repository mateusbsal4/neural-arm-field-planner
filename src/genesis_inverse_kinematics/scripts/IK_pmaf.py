...
class IK_Controller:
    def __init__(self):
        ...
        cam_pose = np.array([[ 0, 0, 1, 2.0],
                             ...
                             [ 0, 1, 0, 0.7],
                             [ 0, 0, 0, 1]])
        self.cam_pose_rviz = np.array([[ 0, 0, -1, 2.0],
                                        [ 1, 0, 0, 0.0],
                                        [ 0, -1, 0, 0.7],
                                        [ 0, 0, 0, 1]])
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.data_received:
                if (self.evaluate or self.inference) and not started_recording:
                ...
                if (np.allclose(ee_pos, self.goal_pos, atol=1e-3) or (planning_time >= 120 and not self.evaluate) 
                or len(self.franka.detect_collision()) > 0):            # planning stops upon reaching the goal position, after 30s (except when evaluating) or if the robot collides
                    ...
                    if self.inference:
                        inference_path = f"/home/geriatronics/pmaf_ws/src/genesis_inverse_kinematics/inference/{self.scene_config}"
                        os.makedirs(inference_path, exist_ok=True)
                        cost_log_path = os.path.join(inference_path, "cost_log.txt")
                        video_path = os.path.join(inference_path, "video.mp4")
                        adjusted_goal = np.array(self.goal_pos) - np.array([0.0, 0.0, 0.10365])
                        distance_from_goal = np.linalg.norm(TCP_pos - adjusted_goal)
                        clearance_cost = costs[0]
                        path_length_cost = costs[1]
                        smoothness_cost = costs[2]
                        goal_deviation_cost = costs[3]
                        with open(cost_log_path, "a") as file:
                            file.write(
                                f"Clearance cost: {clearance_cost:.6f}\n"
                                f"Path length cost: {path_length_cost:.6f}\n"
                                f"Smoothness cost: {smoothness_cost:.6f}\n"
                                f"Goal deviation cost: {goal_deviation_cost:.6f}\n"
                                f"Distance from goal: {distance_from_goal:.6f}\n\n"
                            )
                        self.cam.stop_recording(save_to_filename=video_path, fps=15)
